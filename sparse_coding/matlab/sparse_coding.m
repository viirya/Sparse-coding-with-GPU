...
%     B = rand(pars.patch_size,pars.num_bases);
else
    disp('Using Binit...');
    B = Binit;
end;
...
fid = fopen(pars.filename,'a+');
fprintf(fid,'iepoch, fobj, coeffs_time, basis_time, avg_nnz\n');
...
%            fprintf('sparsity_S = %g\n', sparsity_S);
    ...
    fprintf(fid,'%d, %f, %f, %f, %f\n',t,stat.fobj_avg(t),coeffs_time,basis_time,nnz(S_all)/(size(S_all,1)*size(S_all,2)));
    % display
...
%      fprintf(fid,'%d, %f, %0.2f',t,stat.fobj_avg(t),stat.elapsed_time(t));
...
% %        fprintf('saved as %s\n', experiment.matfname);
%     end
%     profsave(profile('info'),sprintf('profile_results_iter_%d',t));
%     profile clear;
end
fclose(fid);
